...
A.id_anggota=P.id_anggota")->result();
    ...
    {
        if ($this->session->userdata('status') != "login") {
            $alert = $this->session->set_flashdata('alert', 'Anda
belum Login');
            redirect(base_url());
        } else {
            ...
            $o =
                $this->M_perpus->edit_data(array('id_buku' => $id), 'transa
ksi')->num_rows();
            if ($o > 0) {
                $this->session->set_flashdata('alert', 'Buku ini
...
transaksi T,detail_pinjam D, buku B where
T.id_buku=D.id_buku and T.id_pinjam=D.id_pinjam and
T.id_anggota='$where'")->result();
        $this->load->view('desain');
        $this->load->view('toplayout', $data);
